        ...
        if (!transfer.state.isWaiting()) {
            if (++transfer.timeout >= Constants.Network.TIMEOUT) {
                if (++transfer.retries >= Constants.Network.RETRIES) {
                    Reference.logger.warn("{}'s download was dropped!", player.getEntityName());
                    return;
                }

                Reference.logger.warn("{}'s download timed out, retrying (#{})", player.getEntityName(), transfer.retries);
