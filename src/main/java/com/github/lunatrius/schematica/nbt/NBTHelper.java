...
public class NBTHelper {
    ...
    public static List<TileEntity> readTileEntitiesFromCompound(final NBTTagCompound compound, final List<TileEntity> tileEntities) {
        final NBTTagList tagList = compound.getTagList(Names.NBT.TILE_ENTITIES);
        for (int i = 0; i < tagList.tagCount(); i++) {
            final NBTTagCompound tileEntityCompound = (NBTTagCompound) tagList.tagAt(i);
    ...
    public static List<Entity> readEntitiesFromCompound(final NBTTagCompound compound, final World world, final List<Entity> entities) {
        final NBTTagList tagList = compound.getTagList(Names.NBT.ENTITIES);
        for (int i = 0; i < tagList.tagCount(); i++) {
            final NBTTagCompound entityCompound = (NBTTagCompound) tagList.tagAt(i);
